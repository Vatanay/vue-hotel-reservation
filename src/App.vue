...
<div id="app" class="container d-flex w-100 h-100 p-3 mx-auto flex-column">
    ...
    <main class="px-3">
        <Welcome @begin="start" />
        <section class="py-2 text-center container" id="finder" v-bind:class="this.$store.state.wizard_step > 0 ? 'opacity-100 d-block' : 'opacity-0 d-none'">
                                ...
                                <div class="accordion-body row justify-content-center">
                                    <div class="col-6 text-start">
                                        ...
                                        <template v-if="selectedHotel > 0">
                                            ...
                                            <button class="btn btn-primary btn-lg mt-5 d-block mx-auto fw-light" v-if="selectedHotel" v-on:click="$store.state.wizard_step = 2">
                                                Next <i class="bi bi-chevron-right align-bottom"></i>
                                            </button>
                                        </template>
                                    </div>
                                </div>
                                ...
                                <div class="accordion-body row justify-content-center">
                                    <div class="col-6 text-start">
                                        <div class="form-group">
                                            ...
                                            <date-range-picker ref="picker" class="d-block" id="date" v-model="dateRange" :date-format="dateFormat" @update="calculateDays()">
                                    ...
                                    <div class="col-12 mt-3 text-center">
                                        <div class="text-muted small mb-5" v-if="!selectedHotelDetails.child_status">
                                            * This hotel welcomes adult guests only.
                                        </div>
                                        <button class="btn fw-light text-muted" v-on:click="$store.state.wizard_step -= 1">
                                            Back
                                        </button>
                                        <button class="btn btn-primary btn-lg" v-bind:class="totalDate && orderlist.adult > 0 ? '' : 'disabled'" v-on:click="$store.state.wizard_step += 1">
                                            Next
                                        </button>
                                    </div>
                                </div>
                                    ...
                                    <div class="album py-4" v-if="this.$store.state.wizard_step == 3">
                                        <div class="container text-start">
                                                ...
                                                <div class="col" v-for="room in room_type" :key="room.id" v-bind:class="orderlist.room_type !== room.id && orderlist.room_type !== 0? 'opacity-25': ''" v-on:click="selectRoom(room.price, room.price * totalDate * (Number(orderlist.adult) + Number(orderlist.child)),room.id)">
                                                    <div class="card h-100 cursor-pointer">
                                                        ...
                                                        <div class="card-footer text-end">
                                                            ...
                                                            <h4 class="m-0">
                                                                {{room.price * totalDate * (Number(orderlist.adult) + Number(orderlist.child))}} TL
                                                            </h4>
                                                        </div>
                                                    </div>
                                                </div>
                                            ...
                                            <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 g-3">
                                                <div class="col" v-for="scenic in room_scenic" :key="scenic.id" v-bind:class="orderlist.room_scenic !== scenic.id && orderlist.room_scenic !== 0? 'opacity-25': ''" v-on:click="orderlist.room_scenic = scenic.id;selectedPriceRate = scenic.price_rate;">
                                                    <div class="card bg-dark text-white border-0 cursor-pointer">
                                                        ...
                                                        <div class="card-img-overlay text-center d-flex align-items-center">
                                                            ...
                                                            <h4 class="card-title" v-if="selectedRoomPrice > 0">
                                                                {{ scenic.price_rate > 1 ? (selectedRoomPrice * totalDate * (Number(orderlist.adult) + Number(orderlist.child)) + (selectedRoomPrice * totalDate * (Number(orderlist.adult) + Number(orderlist.child)) * scenic.price_rate / 100)).toFixed(2) : selectedRoomPrice * totalDate * (Number(orderlist.adult) + Number(orderlist.child)) }} TL
                                                            </h4>
                                                        </div>
                                                    </div>
                                                </div>
                                            ...
                                            <div class="text-center mt-3">
                                                ...
                                                <button class="btn btn-primary btn-lg" v-bind:class="orderlist.room_type !== 0 && orderlist.room_scenic  ? '' : 'disabled'" v-on:click="$store.state.wizard_step += 1">
                                                    Next
                                                </button>
                                            </div>
                                        </div>
                                        ...
                                        <div class="container text-start">
                                            <div class="row">
                                                    ...
                                                    <div class="col-10 mx-auto">
                                                        ...
                                                        <table class="border-0 small mx-auto w-100">
                                                            ...
                                                            <tr v-if="coupon.result">
                                                                <td class="text-start"><b>Discount ({{coupon.code}})</b></td>
                                                ...
                                                <div class="col-12 mt-3 text-center">
                                                    ...
                                                    <button class="btn btn-primary btn-lg" v-on:click="finish()">
                                                        Pay {{price().toFixed(2)}} TL
                                                    </button>
                                                </div>
                                            </div>
                                        </div>
                                            ...
                                            <div class="row">
                                                <div class="col-12 col-sm-6 me-auto">
                                                    <div class="feature">
                                                        ...
                                                        <a v-on:click="clear();" class="cursor-pointer icon-link text-decoration-none">
                                                            New reservation <i class="bi bi-chevron-right small"></i>
                                                        </a>
                                                    </div>
                                                </div>
...
export default {
    ...
    methods: {
        ...
        useCoupon() {
            axios.get("https://5f6d939160cf97001641b049.mockapi.io/tkn/coupons?code=" + this.coupon.code)
                .then(response => {
        ...
        finish() {
            ...
            axios.post("https://5f6d939160cf97001641b049.mockapi.io/tkn/hotel-bookings", this.orderlist)
                .then(response => {
    ...
    async created() {
        let response = await axios.get('https://5f6d939160cf97001641b049.mockapi.io/tkn/hotels');
        this.hotels = response.data;

        response = await axios.get('https://5f6d939160cf97001641b049.mockapi.io/tkn/hotel-details');
...
body {
    ...
    main {
        .accordion-item {
            border: 0;
            box-shadow: rgb(108 105 108 / 10%) 0 2px 2px, rgb(108 105 108 / 10%) 0 4px 4px, rgb(108 105 108 / 10%) 0 8px 8px, rgb(108 105 108 / 10%) 0 16px 16px, rgb(108 105 108 / 10%) 0 32px 32px, rgb(108 105 108 / 10%) 0 64px 64px;
        }
